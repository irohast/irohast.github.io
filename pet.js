...
$(document).ready(function(){
    ...
    $.getJSON("./pets.json", function(data){
        ...
        // ㄱㄴㄷ 정렬
        pet.sort((a,b)=> a.name.localeCompare(b.name,'ko-KR'));
...
function initAutoComplete(){
    ...
    const box = $('<ul id="suggestBox"></ul>').css({
        position:'absolute',
        background:'#fff',
    ...
    $("#name").on("input",function(){
        ...
        const list = pet.filter(p=>p.name.includes(val)).slice(0,10);

        list.forEach(p=>{
            const li=$("<li>"+p.name+"</li>").css({
                padding:'6px',
                cursor:'pointer'
            });
...
$("#name").on("change",function(){
    const p=pet.find(x=>x.name===$(this).val());
    if(p) showSRank(p);
});

function showSRank(p){
    const s=calcSRank(p);
...
$("#analyze").on("click",function(){
    ...
    const atk=parseFloat($("#catk").val())||0;
    const def=parseFloat($("#cdef").val())||0;
    const agi=parseFloat($("#cagi").val())||0;

    const sum=atk+def+agi;

    const probability=Math.min(100,(sum/lvl/3.8)*100);
