...
function setSRank(copiedPet) {
    ...
    // 珥덇린移  怨꾩궛 (base 2 + bonus 2.5瑜   ⑹궛 섏뿬 4.5  곸슜)
    ...
    //  뚯씠釉   쎌엯 (id="srank-label" 遺   )
    $('#myPet tbody').append(''
        + '<tr class="srank">'
        + '<td id="srank-label" style="vertical-align: middle;">S 珥덇린移 </td>'
...
function print() {
    ...
    let message = '';
    let sRankRate = "0.00"; // 湲곕낯媛   ㅼ젙
    const totalCount = result.length;

    if (btnName == "search") {
        const name = $("#name").val();
        message += `[${name}]   8 깃툒 遺꾪룷    ㅼ쓬怨  媛숈뒿 덈떎!`;
        sRankRate = "怨꾩궛以 ";
        ...
        // 8 깃툒   議댁옱 섍퀬  꾩껜 寃곌낵媛   덉쓣  뚮쭔  뺣쪧 怨꾩궛
        ...
        message += `[${$("#name").val()}] 怨꾩궛  꾨즺!\n`;
        ranks.forEach(rank => {
            let per = (rankCounts[rank] / totalCount * 100).toFixed(2);
            message += `${rank}  깃툒: ${rankCounts[rank]} 嫄  (${per}%) \n`;
        });
    }

    // 8 깃툒   0嫄댁씠 대룄 (8 깃툒: 0.00%) 쇨퀬  뺤긽  쒖떆 ⑸땲  .
    $("#srank-label").html('S 珥덇린移 <br><span style="color:#d9534f; font-size:11px; font-weight:bold;">(8 깃툒: ' + sRankRate + '%)</span>');
