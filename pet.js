// ===== 전역 (ui.js와 공유) =====
if(typeof pet === "undefined") var pet = [];
if(typeof btnName === "undefined") let btnName = "";
if(typeof result === "undefined") const result = [];
...
function addData(foundPet, base, bonus) {
    ...
    let stat = {"rank":0,"hp":0,"atk":0,"def":0,"agi":0,
                "baseHp":0,"baseAtk":0,"baseDef":0,"baseAgi":0,
                "bonusHp":0,"bonusAtk":0,"bonusDef":0,"bonusAgi":0};
...
function print() {
    ...
    for(let i=0;i<result.length;i++){
        $('#result tbody').append(
            ...
            <td>${i+1}</td>
            <td>${result[i].rank}</td>
            <td>${limitDecimalPlaces(result[i].hp,2)}</td>
            <td>${limitDecimalPlaces(result[i].atk,2)}</td>
            <td>${limitDecimalPlaces(result[i].def,2)}</td>
            <td>${limitDecimalPlaces(result[i].agi,2)}</td>
            </tr>`
        );
        if(i>=299) break;
    }
